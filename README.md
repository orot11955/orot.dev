...
OROT.DEV는 개발, 사진, 기록을 공개하는 개인 사이트이자 글 작성, 검수, 발행, 댓글 관리, 사진 갤러리 운영까지 한 흐름으로 다루는 콘텐츠 CMS 프로젝트입니다.  
공개 웹사이트는 Next.js로 제공하고, 내부 Editor/Studio와 운영 API는 NestJS + Prisma 기반으로 구성해 개인 퍼블리싱과 콘텐츠 운영 도구를 함께 관리합니다.
...
- [.env.development.example](.env.development.example): 기본 개발 템플릿
...
- [.env.production.example](.env.production.example): 운영/Compose 템플릿
...
```bash
yarn docker:demo:up
yarn docker:demo:logs
yarn docker:demo:down
```
...
- 개발 기본 템플릿: [.env.development.example](.env.development.example)
...
- 루트 `.env.production`
- 시작 템플릿은 [.env.production.example](.env.production.example)입니다.
