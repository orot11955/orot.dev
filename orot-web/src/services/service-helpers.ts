...
export async function listResource<TItem>(
  ...
  if (isDemoMode()) {
    const data = await demoGet<ApiListPayload<TItem>>(
      path,
...
export function createResource<T, B = unknown>(
  ...
  if (isDemoMode()) {
    return demoPost<T, B>(path, payload);
  }
...
export function patchResource<T, B = unknown>(
  ...
  if (isDemoMode()) {
    return demoPatch<T, B>(path, payload);
  }
...
export function postFormResource<T>(
  ...
  if (isDemoMode()) {
    return demoUploadImage<T>(path, entries);
  }
